...
const EnlightedCards = ({ budget, next }: Props) => {
  return (
    <div className="flex flex-col gap-y-5 rounded-[0.75rem] bg-white px-5 py-6 sm:p-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-x-4">
          <div className={clsx("size-4 rounded-full bg-red-600", budget.color)} />
      ...
      <div className="flex flex-col gap-y-4">
        <p className="text-preset-4 text-gray-500">Maximum of ${budget.maxAmount}</p>
        <Slider
          sliderBorderClass="p-1"
          sliderPipeClass={`${budget.color} h-6 `}
          value={Math.round((budget.spent / budget.maxAmount) * 100)}
        />

        <div className="flex items-center justify-between gap-x-4">
          <div className="flex w-1/2 items-center gap-x-4">
            <div className={clsx("h-[2.69rem] w-1 rounded-[0.5rem] bg-green-800", budget.color)} />
            <div className="flex flex-col justify-between">
              <p className="text-preset-5 text-gray-500">Spent</p>
              <p className="text-preset-4-bold">${budget.spent}</p>
            </div>
          </div>
          <div className="flex w-1/2 items-center justify-start gap-x-4">
            ...
            <div className="flex flex-col justify-between">
              ...
              <p className="text-preset-4-bold">${budget.currentAmount}</p>
            </div>
          </div>
        </div>
      </div>
