...
const DeleteDialog = ({ description, title, name }: DeleteDialogProps) => {
  return (
    <AlertDialog>
      <AlertDialogTrigger className="text-preset-4 flex h-5 items-center justify-center rounded-md bg-white p-0 font-normal text-nowrap text-black">
        {name}
      </AlertDialogTrigger>
      <AlertDialogContent className="mx-auto flex h-auto max-h-[32rem] w-[calc(100vw-2rem)] flex-col justify-between gap-y-5 px-5 py-6 sm:w-full sm:max-w-[35rem] sm:p-8">
        <AlertDialogHeader className="h-min w-full gap-y-5">
          ...
          <div className="text-preset-4 flex h-min flex-col justify-between gap-y-5">
            <AlertDialogDescription className="text-preset-4 h-min text-start text-gray-500">{description}</AlertDialogDescription>
          </div>
        </AlertDialogHeader>
